...
namespace Assets.Scripts.AdvancedCity
{
    class BlockGeneratorBasic : IBlockGenerator
    {
        ...
        public void GenerateBuildings(Vector3[] vertexes, IObjectGenerator objectGenerator)
        {
            Clear();
            objectGenerator.CreateGreenPlace(vertexes);
            ...
            for (int i = 0; i < vertexes.Length; i++)
            {
                ...
                {
                    buildings.Add(objectGenerator.CreateBuilding(controlPoints));
                }
                else
                {
                    succes = false;
            ...
            {
                MakeSideBuildings(lines[i], objectGenerator);
            }
            if (!succes)
                Clear();
        }
        ...
        private void MakeSideBuildings(ControlLine controlLine, IObjectGenerator objectGenerator)
        {
            ...
            if (length > values.MinHouse * 2)
            {
                ...
                MakeSideBuildings(new ControlLine(controlLine.prevSidePoint, controlLine.prevDeepPoint, tmpNormal, tmpDeep), objectGenerator);
            }
            else
            {
            ...
            buildings.Add(objectGenerator.CreateBuilding(kontrolpoints.ToArray()));
        }
    }
}
